    ...
    if(term) {
        ...
        if(results.length>0){
            ...
            for (var i = 0; i < results.length; i++) {
                ...
                var body = documents[ref]['body'].substring(0, 60)+'...';
...
function lunr_search(term) {
    ...
    if(term) {
        document.getElementById('modtit').innerHTML = "<h5 class='modal-title'>'" + term + "' 的搜索结果</h5>" + document.getElementById('modtit').innerHTML;
        //put results on the screen.
        var results = idx.search(term);
        if(results.length > 0){
            //console.log(idx.search(term));
            ...
            for (var i = 0; i < results.length; i++) {
                var ref = results[i]['ref'];
                var date = new Date(documents[ref]['date']);
                console.log(date);
